...
function levelCommission($userId, $remainingPoints, $deductedPoints, $trx, $commissionType = '')
{
    ...
    $refId = $user->referred_by;
    $i = 1;
    $levelCount = ReferralSetting::where('commission_type', $commissionType)->count();
    
    while ($i <= $levelCount && $refId) {
        ...
        $refUser->balance += $commissionAmount;
        $refUser->save();
        ...
        $transactions[] = [
            ...
            'trx_type' => '+',
            'details' => 'Level ' . $i . ' referral commission From ' . $user->email,
            'remark' => 'referral',
            'trx' => $trx,
            'created_at' => now()
        ];
        ...
        $commissionLog[] = [
            ...
            'trx_amo' => $remainingPoints,
            'title' => 'Level ' . $i . ' referral commission from ' . $user->email,
            'type' => $commissionType,
