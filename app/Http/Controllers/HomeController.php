...
class HomeController extends Controller
{
    ...
    public function referralRegistrationPost(Request $request)
    {
        $request->validate([
            'referBy' => 'required|string|exists:users,referral_code',
            'name' => 'required|string|max:255',
            'email' => 'required|string|email|max:255|unique:users,email',
            'password' => 'required|string|min:8|confirmed',
        ]);

        $ref = User::where('referral_code', $request->referBy)->first();

        if ($ref) {
            $refBy = $ref->id;
        } else {
            return redirect()->back()->withErrors(['referBy' => 'Invalid referral code.'])->withInput();
        }

        $user = new User();
        $user->referred_by = $refBy;
        $user->name = $request->name;
        $user->email = $request->email;
        $user->password = Hash::make($request->password);
        $user->referral_code = $this->generateReferralCode();
        $user->email_verified_at = '2024-03-18 04:14:37';
        //dd($user);
        $user->save();
    ...
    public function referralRegistrationView(Request $request, $reference = null)
    {
        if (Auth::check()) {
            return redirect()->route('home');
        }
        ...
        return view('frontend.user_registration_referral');
    }
